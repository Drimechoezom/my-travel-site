...
typography:
  display-xl:
    fontFamily: '"Plain Black", "Smiley Sans", "STYuanti", "Yuanti SC", "YouYuan", "PingFang SC", "Noto Sans SC", "Microsoft YaHei", sans-serif'
    fontSize: 72px
    fontWeight: 500
    lineHeight: 1
    letterSpacing: -2.5px
...
## Typography & Bilingual Font System (字体与中英双语系统)

### Font Family & Bilingual Stacks (中英双语字体栈)

The site is bilingual (Chinese & English). To preserve the playful, warm, rounded display voice of **Plain Black** and the clean modern tone of **Inter**, the fallback font stacks prioritize English font faces first, followed by carefully matched Chinese font families across platforms (macOS/iOS, Windows, Android, WebFonts).

#### 1. Display / Headlines Stack (大字号标题与展陈字体)
Matches the warm, rounded character of **Plain Black** (亲和、质感圆润、富有生命力的展陈大字):
- **Stack**: `"Plain Black", "Smiley Sans", "STYuanti", "Yuanti SC", "YouYuan", "PingFang SC", "Noto Sans SC", "Microsoft YaHei", sans-serif`
- **Chinese Character Choices (中文字体选型理由)**:
  - **Smiley Sans (钉钉进步体 / 矢量潮牌圆体)**: High-energy, rounded vector display typeface. Perfect webfont match for English Plain Black.
  - **STYuanti / Yuanti SC / YouYuan (华文幼圆 / 苹方圆体 / 幼圆)**: System rounded sans-serifs that bring warm clay-like friendly curves on macOS and Windows.
  - **PingFang SC & Noto Sans SC (苹方 & 思源黑体)**: Modern geometric fallbacks with medium weight (`fontWeight: 500`).

#### 2. Body, UI & Navigation Stack (正文、界面与导航字体)
Matches the clean, functional precision of **Inter**:
- **Stack**: `"Inter", "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", "Noto Sans SC", sans-serif`
- **Chinese Character Choices**:
  - **PingFang SC (苹方)**: Primary CJK body font for macOS & iOS, providing crystal clear rendering and elegant proportional rhythm.
  - **Microsoft YaHei (微软雅黑)**: Standard Windows fallback for high-legibility screen reading.
  - **Noto Sans SC / Source Han Sans SC (思源黑体)**: Webfont & Android fallback for consistent cross-platform body typesetting.

---

### Hierarchy & Bilingual Specifications (字号层级与中英双语对比)

| Token | Size | Weight | Line Height (En / Zh) | Letter Spacing (En / Zh) | Use |
|---|---|---|---|---|---|
| `{typography.display-xl}` | 72px | 500 | 1.0 / 1.1 | -2.5px / 0px | Homepage h1 hero headline — Plain Black / Smiley Sans |
| `{typography.display-lg}` | 56px | 500 | 1.05 / 1.15 | -2px / 0px | Section heads — Plain Black / STYuanti |
| `{typography.display-md}` | 40px | 500 | 1.1 / 1.2 | -1px / 0px | Sub-section heads, destination title |
| `{typography.display-sm}` | 32px | 500 | 1.15 / 1.25 | -0.5px / 0px | CTA-band heads, feature card titles |
| `{typography.title-lg}` | 24px | 600 | 1.3 / 1.35 | -0.3px / 0px | Card titles, article section heads |
| `{typography.title-md}` | 18px | 600 | 1.4 / 1.45 | 0px / 0px | Card sub-titles, travel log intros |
| `{typography.title-sm}` | 16px | 600 | 1.4 / 1.45 | 0px / 0px | Meta tags, location labels |
| `{typography.body-md}` | 16px | 400 | 1.55 / 1.65 | 0px / 0px | Default running text / travel log content |
| `{typography.body-sm}` | 14px | 400 | 1.55 / 1.65 | 0px / 0px | Captions, dates, footer copy |
| `{typography.caption}` | 13px | 500 | 1.4 / 1.4 | 0px / 0px | Badge pills, status tags |
| `{typography.caption-uppercase}`| 12px | 600 | 1.4 / 1.4 | 1.5px / 0.5px | Section labels, uppercase tags ("FEATURED") |
| `{typography.button}` | 14px | 600 | 1.0 / 1.2 | 0px / 0px | Button labels, interactive CTAs |
| `{typography.nav-link}` | 14px | 500 | 1.4 / 1.4 | 0px / 0px | Top-nav links |

---

### Chinese Typesetting & CJK Rules (中文字体排版规范)

1. **Letter Spacing Reset (字间距微调原则)**:
   - English headlines use tight negative letter-spacing (`-2.5px` to `-1px`). **CJK ideographs MUST NOT use negative letter-spacing**, as Chinese characters are fixed-width squares; negative spacing causes character collision.
   - In CSS, apply `letter-spacing: 0` or `letter-spacing: normal` when rendering Chinese text (`:lang(zh)` or CJK headline containers).

2. **Line Height Adjustments for Chinese Body (行高增幅)**:
   - Chinese characters are denser than Latin alphabets. Increase `line-height` from `1.55` (English default) to `1.65` - `1.75` for body text (`{typography.body-md}`) to ensure optimal readability for long travel logs.

3. **Mixed Typesetting & Spacing (中英文混排)**:
   - Insert half-width spaces between CJK characters and Latin alphabets / Western numbers (e.g., `在 Tokyo 度过 7 天` instead of `在Tokyo度过7天`).
   - Use CSS property `text-autospace: ideograph-alpha;` where supported, or manual spacing.

4. **CJK Font Weight Mapping (跨平台字重映射)**:
   - `fontWeight: 500` (Medium) maps to `PingFang SC Medium` on macOS/iOS, and falls back cleanly to standard bold or default rendering on Windows (`Microsoft YaHei`).
   - `fontWeight: 600` (Semibold) provides clear visual punch without looking overly heavy.

5. **Punctuation Rules (中文标点规范)**:
   - Use full-width Chinese punctuation for Chinese text (`，`, `。`, `！`, `？`, `《`, `》`).
   - Quotes in Chinese travel stories should use `“ ”` (弯引号) or `「 」` (直角引号).

### Note on Font Substitutes (字体替代建议)
- **Display Substitute (标题替代方案)**: If `Plain Black` is unavailable, `Smiley Sans` (开源免费商用) or `Yuanti SC / STYuanti` (系统幼圆) provides the exact rounded display warmth needed for the Clay aesthetic.
- **Body Substitute (正文替代方案)**: `Inter` paired with `PingFang SC` / `Microsoft YaHei` ensures crisp, high-density reading across all modern desktop and mobile browsers.
